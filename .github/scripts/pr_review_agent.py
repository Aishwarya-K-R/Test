            ...
            for issue in issues:
                ...
                try:
                    ...
                    if fixed and fixed.strip() != working_content.strip():
                        # Safety gate: reject if too many lines changed (likely a full rewrite)
                        original_lines = working_content.splitlines()
                        fixed_lines    = fixed.splitlines()
                        lines_changed  = sum(1 for a, b in zip(original_lines, fixed_lines) if a != b)
                        lines_changed += abs(len(fixed_lines) - len(original_lines))
                        if lines_changed > 15:
                            print(f"    → Skipped '{issue['title']}': change too large ({lines_changed} lines) — apply manually")
                        else:
